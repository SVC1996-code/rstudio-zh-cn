...
- 修复 r2 中启动恢复 Data Viewer 标签页时可能出现白屏的本地化回归。
- 内嵌窗口尚未初始化时显示注入执行过早；新增 window/document 存在检查，无轮询或异常吞噬。
- 验证新建与重启恢复 Data Viewer 均正常，不改变翻译内容、用户数据或 RStudio 内部 ID。

## 2026-09-16 — v2026.09.0+174-zh_CN-r2

- 扩充 Keyboard Shortcuts 中文显示名，补充 Workbench 命令及范围名称中文。
- 完善 Data Viewer 状态、统计、排序、筛选、固定列和摘要帮助等中文。
- 保持 command ID、快捷键绑定、内部 scope、用户数据及 R 类型标识不变。
- 保留 r1 的 Windows PowerShell 5.1 支持和双击安装体验。
